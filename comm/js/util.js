;(function (window){
	...
	$.fn.fillFormField = function(json) {
	...
	Omega.util.dom = {
		 ...
		 * @returns {number}
		 * @example Omega.util.dom.getClientWidth(document);
		 */
		 ...
		 * @param {object} [doc=document] 所需检查的页面document引用
		 * @returns {number}
		 * @example Omega.util.dom.getClientHeight(document);
		 */
		 ...
		 * @param {object} [doc=document] 所需检查的页面document引用
		 * @returns {number}
		 * @example Omega.util.dom.getScrollHeight(document);
		 */
		 ...
		 * @param {object} [doc=document] 所需检查的页面document引用
		 * @returns {number}
		 * @example Omega.util.dom.getScrollLeft(document);
		 */
		 ...
		 * @param {object} [doc=document] 所需检查的页面document引用
		 * @returns {number}
		 * @example Omega.util.dom.getScrollTop(document);
		 */
	...
	Omega.util.cookie = {
		 ...
		 * @example
		 * 		Omega.util.cookie.get('value1'); //获取cookie
		 */
		get : function(name) {
	...
	Omega.util.acc = {
		...
		add : function (arg1, arg2) {
		    ...
		    return (parseInt(Omega.util.acc.mul(arg1, m), 10) + parseInt(Omega.util.acc.mul(arg2, m), 10)) / m;
		...
		sub : function (arg1, arg2) {
			...
			m = Math.pow(10, Math.max(r1, r2));
			n = (r1 >= r2) ? r1 : r2;
			return ((Omega.util.acc.mul(arg1, m) - Omega.util.acc.mul(arg2, m)) / m).toFixed(n);
	 ...
	 */
	Omega.util.getToken = function (name) {
		var str = Omega.util.cookie.get(name || 'userCode');
		var hash = 5381;
	 ...
	 * @return {String} 新URL
	 */
	Omega.util.appendUrlParam = function (url, param, forHash) {
		if (!url) {
			return '';
		}
		if (!param) {
	 ...
	 * @return {String} 参数值
	 */
	Omega.util.getUrlParam = function (name) {
	 ...
	 */
	Omega.util.toThousands = function(num){
	 ...
	 * @return {Number} 字符串长度
	 */
	Omega.util.getStrLength = function(str){
		str = str || '';
		str = str.toString();
	 ...
	 */
	Omega.util.formatCTNum = function(num){
	 ...
	 */
	Omega.util.formatDate = function (time, fmt, forEng) {
		if(!time){
			return time;
		}
	 ...
	 */
	Omega.util.Arabia2Chinese = function (Num) {
		Num = Num || '0';
		Num = Num.toString();
		if(Number(Num)==0) {
			return "";
